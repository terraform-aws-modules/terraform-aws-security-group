provider "aws" {
  region = "eu-west-1"

  skip_credentials_validation = true
  skip_requesting_account_id  = true
  skip_metadata_api_check     = true
  skip_region_validation      = true
}

#############################################################
# Data sources to get VPC and default security group details
#############################################################
data "aws_vpc" "default" {
  default = true
}

data "aws_security_group" "default" {
  name   = "default"
  vpc_id = data.aws_vpc.default.id
}

##################################################
# VPC which is used as an argument in complete-sg
##################################################
module "vpc" {
  source  = "terraform-aws-modules/vpc/aws"
  version = "~> 5.0"

  name = "complete-sg-demo-vpc"
  cidr = "10.20.0.0/20"
}

#############################################################
# Security group which is used as an argument in complete-sg
#############################################################
module "main_sg" {
  source = "../../"

  name        = "main-sg"
  description = "Security group which is used as an argument in complete-sg"
  vpc_id      = data.aws_vpc.default.id

  ingress_cidr_ipv4 = ["10.10.0.0/16"]
  ingress_rules     = ["https-443-tcp"]
}

################################################
# Security group with complete set of arguments
################################################
module "complete_sg" {
  source = "../../"

  name        = "complete-sg"
  description = "Security group with all available arguments set (this is just an example)"
  vpc_id      = data.aws_vpc.default.id

  tags = {
    Cash       = "king"
    Department = "kingdom"
  }

  # Default CIDR blocks, which will be used for all ingress rules in this module. Typically these are CIDR blocks of the VPC.
  # If this is not specified then no CIDR blocks will be used.
  ingress_cidr_ipv4 = ["10.10.0.0/16"]

  ingress_cidr_ipv6 = ["2001:db8::/64"]

  # Prefix list ids to use in all ingress rules in this module.
  # ingress_prefix_list_ids = [data.aws_prefix_list.s3.id, data.aws_prefix_list.dynamodb.id]

  # Open for all CIDRs defined in ingress_cidr_ipv4
  ingress_rules = ["https-443-tcp"]

  # Use computed value here (eg, `${module...}`). Plain string is not a real use-case for this argument.
  computed_ingress_rules           = ["ssh-tcp"]
  number_of_computed_ingress_rules = 1

  # Open to CIDRs blocks (rule or from_port+to_port+ip_protocol+description)
  ingress_with_cidr_ipv4 = [
    {
      rule      = "postgresql-tcp"
      cidr_ipv4 = "0.0.0.0/0"
    },
    {
      rule      = "postgresql-tcp"
      cidr_ipv4 = "30.30.30.30/32"
    },
    {
      from_port   = 10
      to_port     = 20
      ip_protocol = 6
      description = "Service name"
      cidr_ipv4   = "10.10.0.0/20"
    },
  ]

  computed_ingress_with_cidr_ipv4 = [
    {
      rule      = "postgresql-tcp"
      cidr_ipv4 = "3.3.3.3/32"
    },
    {
      from_port   = 15
      to_port     = 25
      ip_protocol = 6
      description = "Service name with vpc cidr"
      cidr_ipv4   = module.vpc.vpc_cidr_block
    },
  ]

  number_of_computed_ingress_with_cidr_ipv4 = 2

  # Open to IPV6 CIDR blocks (rule or from_port+to_port+ip_protocol+description)
  ingress_with_cidr_ipv6 = [
    {
      from_port   = 300
      to_port     = 400
      ip_protocol = "tcp"
      description = "Service ports (ipv6)"
      cidr_ipv6   = "2001:db8::/64"
    },
  ]

  computed_ingress_with_cidr_ipv6 = [
    {
      from_port   = 350
      to_port     = 450
      ip_protocol = "tcp"
      description = "Service ports (ipv6). VPC ID = ${module.vpc.vpc_id}"
      cidr_ipv6   = "2001:db8::/64"
    },
  ]

  number_of_computed_ingress_with_cidr_ipv6 = 1

  # Open for security group id (rule or from_port+to_port+ip_protocol+description)
  ingress_with_referenced_security_group_id = [
    {
      rule                         = "mysql-tcp"
      referenced_security_group_id = data.aws_security_group.default.id
    },
    {
      from_port                    = 10
      to_port                      = 10
      ip_protocol                  = 6
      description                  = "Service name"
      referenced_security_group_id = data.aws_security_group.default.id
    },
  ]

  computed_ingress_with_referenced_security_group_id = [
    {
      rule                         = "postgresql-tcp"
      referenced_security_group_id = module.main_sg.security_group_id
    },
    {
      from_port                    = 23
      to_port                      = 23
      ip_protocol                  = 6
      description                  = "Service name"
      referenced_security_group_id = module.main_sg.security_group_id
    },
  ]

  number_of_computed_ingress_with_referenced_security_group_id = 2

  # Open for self (rule or from_port+to_port+ip_protocol+description)
  ingress_with_self = [
    {
      rule = "all-all"
    },
    {
      from_port   = 30
      to_port     = 40
      ip_protocol = 6
      description = "Service name"
    },
    {
      from_port   = 41
      to_port     = 51
      ip_protocol = 6
    },
  ]

  computed_ingress_with_self = [
    {
      from_port   = 32
      to_port     = 43
      ip_protocol = 6
      description = "Service name. VPC ID: ${module.vpc.vpc_id}"
    },
  ]

  number_of_computed_ingress_with_self = 1

  # Default CIDR blocks, which will be used for all egress rules in this module. Typically these are CIDR blocks of the VPC.
  # If this is not specified then no CIDR blocks will be used.
  egress_cidr_ipv4 = ["10.10.0.0/16"]

  egress_cidr_ipv6 = ["2001:db8::/64"]

  # Prefix list ids to use in all egress rules in this module.
  # egress_prefix_list_ids = ["pl-123456"]
  # Open for all CIDRs defined in egress_cidr_ipv4
  egress_rules = ["http-80-tcp"]

  computed_egress_rules           = ["ssh-tcp"]
  number_of_computed_egress_rules = 1

  # Open to CIDRs blocks (rule or from_port+to_port+ip_protocol+description)
  egress_with_cidr_ipv4 = [
    {
      rule      = "postgresql-tcp"
      cidr_ipv4 = "0.0.0.0/0"
    },
    {
      rule      = "https-443-tcp"
      cidr_ipv4 = "30.30.30.30/32"
    },
    {
      from_port   = 10
      to_port     = 20
      ip_protocol = 6
      description = "Service name"
      cidr_ipv4   = "10.10.0.0/20"
    },
  ]

  computed_egress_with_cidr_ipv4 = [
    {
      rule      = "https-443-tcp"
      cidr_ipv4 = module.vpc.vpc_cidr_block
    },
  ]

  number_of_computed_egress_with_cidr_ipv4 = 1

  # Open to IPV6 CIDR blocks (rule or from_port+to_port+ip_protocol+description)
  egress_with_cidr_ipv6 = [
    {
      from_port   = 300
      to_port     = 400
      ip_protocol = "tcp"
      description = "Service ports (ipv6)"
      cidr_ipv6   = "2001:db8::/64"
    },
  ]

  computed_egress_with_cidr_ipv6 = [
    {
      from_port   = 55
      to_port     = 66
      ip_protocol = "tcp"
      description = "Service ports (ipv6). VPC ID: ${module.vpc.vpc_id}"
      cidr_ipv6   = "2001:db8::/64"
    },
  ]

  number_of_computed_egress_with_cidr_ipv6 = 1

  # Open for security group id (rule or from_port+to_port+ip_protocol+description)
  egress_with_referenced_security_group_id = [
    {
      rule                         = "mysql-tcp"
      referenced_security_group_id = data.aws_security_group.default.id
    },
    {
      from_port                    = 10
      to_port                      = 10
      ip_protocol                  = 6
      description                  = "Service name"
      referenced_security_group_id = data.aws_security_group.default.id
    },
  ]

  computed_egress_with_referenced_security_group_id = [
    {
      rule                         = "postgresql-tcp"
      referenced_security_group_id = module.main_sg.security_group_id
    },
  ]

  number_of_computed_egress_with_referenced_security_group_id = 1

  # Open for self (rule or from_port+to_port+ip_protocol+description)
  egress_with_self = [
    {
      rule = "all-all"
    },
    {
      from_port   = 30
      to_port     = 40
      ip_protocol = "tcp"
      description = "Service name"
    },
    {
      from_port   = 41
      to_port     = 51
      ip_protocol = 6
    },
  ]

  computed_egress_with_self = [
    {
      rule = "https-443-tcp"
    },
  ]

  number_of_computed_egress_with_self = 1

  create_timeout = "15m"
  delete_timeout = "45m"
}

######################################################
# Security group with IPv4 and IPv6 sets of arguments
######################################################
module "ipv4_ipv6_example" {
  source = "../../"

  name        = "ipv4-ipv6-example"
  description = "IPv4 and IPv6 example"
  vpc_id      = data.aws_vpc.default.id

  ingress_with_cidr_ipv4 = [
    {
      from_port   = 8080
      to_port     = 8090
      ip_protocol = "tcp"
      description = "User-service ports (ipv4)"
      cidr_ipv4   = "0.0.0.0/0"
    },
  ]

  ingress_with_cidr_ipv6 = [
    {
      from_port   = 8080
      to_port     = 8090
      ip_protocol = "tcp"
      description = "User-service ports (ipv6)"
      cidr_ipv6   = "2001:db8::/64"
    },
  ]

  egress_with_cidr_ipv4 = [
    {
      from_port   = 8090
      to_port     = 8100
      ip_protocol = "tcp"
      description = "User-service ports (ipv4)"
      cidr_ipv4   = "0.0.0.0/0"
    },
  ]

  egress_with_cidr_ipv6 = [
    {
      from_port   = 8090
      to_port     = 8100
      ip_protocol = "tcp"
      description = "User-service ports (ipv6)"
      cidr_ipv6   = "2001:db8::/64"
    },
  ]
}

#################################
# Security group with fixed name
#################################
module "fixed_name_sg" {
  source = "../../"

  name        = "fixed-name-sg"
  description = "Security group with fixed name"
  vpc_id      = data.aws_vpc.default.id

  use_name_prefix = false

  ingress_cidr_ipv4 = ["10.10.0.0/16"]
  ingress_rules     = ["https-443-tcp"]
}

############################
# Only security group rules
############################
module "only_rules" {
  source = "../../"

  create_sg         = false
  security_group_id = module.complete_sg.security_group_id
  ingress_with_referenced_security_group_id = [
    {
      description                  = "http from service one"
      rule                         = "http-80-tcp"
      referenced_security_group_id = data.aws_security_group.default.id
    },
  ]
}

###################################
# Security group with prefix lists
###################################

data "aws_prefix_list" "s3" {
  filter {
    name   = "prefix-list-name"
    values = ["com.amazonaws.eu-west-1.s3"]
  }
}

data "aws_prefix_list" "dynamodb" {
  filter {
    name   = "prefix-list-name"
    values = ["com.amazonaws.eu-west-1.dynamodb"]
  }
}

module "prefix_list" {
  source = "../../"

  name        = "pl-sg"
  description = "Security group with prefix list"
  vpc_id      = data.aws_vpc.default.id

  ingress_prefix_list_ids = [data.aws_prefix_list.s3.id, data.aws_prefix_list.dynamodb.id]
  ingress_with_prefix_list_id = [
    {
      from_port      = 9100
      to_port        = 9100
      ip_protocol    = 6 # "tcp"
      description    = "Arbitrary TCP port"
      prefix_list_id = data.aws_prefix_list.s3.id
    },
  ]
}

#################################
# Security group using prefix list
#################################
resource "aws_ec2_managed_prefix_list" "prefix_list_sg_example" {
  address_family = "IPv4"
  max_entries    = 1
  name           = "prefix-list-sg-example"

  entry {
    cidr        = module.vpc.vpc_cidr_block
    description = "VPC CIDR"
  }
}

module "prefix_list_sg" {
  source = "../../"

  name        = "prefix-list-sg"
  description = "Security group using prefix list and custom ingress rules"
  vpc_id      = data.aws_vpc.default.id

  use_name_prefix = false

  ingress_prefix_list_ids = [aws_ec2_managed_prefix_list.prefix_list_sg_example.id]
  ingress_with_prefix_list_id = [
    {
      from_port      = 80
      to_port        = 80
      ip_protocol    = "tcp"
      prefix_list_id = data.aws_prefix_list.s3.id
    },
  ]
}
