module "sg" {
  ...
  ###################
  # Rules by names - open for default CIDR
  computed_ingress_rules = [sort(
    distinct(
      concat(var.auto_computed_ingress_rules, var.computed_ingress_rules),
    ),
  )]

  # Open for self
  computed_ingress_with_self = [concat(
  ...
  ##################
  # Rules by names - open for default CIDR
  computed_egress_rules = [sort(
    distinct(
      concat(var.auto_computed_egress_rules, var.computed_egress_rules),
    ),
  )]

  # Open for self
  computed_egress_with_self = [concat(
